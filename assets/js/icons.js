// MONTIVA — minimalist çizgisel (line) ikon seti. Emoji yerine kullanılır.
const ICONS = {
  ...
  phone: '<path d="M13.83 16.57a1 1 0 0 0 1.21-.3l.36-.47A2 2 0 0 1 17 15h3a2 2 0 0 1 2 2v3a2 2 0 0 1-2 2A18 18 0 0 1 2 4a2 2 0 0 1 2-2h3a2 2 0 0 1 2 2v3a2 2 0 0 1-.8 1.6l-.47.35a1 1 0 0 0-.29 1.24 14 14 0 0 0 6.39 6.38"/>',
  ...
  cursor: '<path d="M12.59 12.59 19 19"/><path d="M3.69 3.04a.5.5 0 0 0-.65.65l6.5 16a.5.5 0 0 0 .95-.06l1.57-6.09a2 2 0 0 1 1.44-1.48l6.13-1.58a.5.5 0 0 0 .06-.95z"/>',
  ...
  support: '<circle cx="12" cy="12" r="10"/><circle cx="12" cy="12" r="4"/><path d="m4.93 4.93 4.24 4.24"/><path d="m14.83 9.17 4.24-4.24"/><path d="m14.83 14.83 4.24 4.24"/><path d="m9.17 14.83-4.24 4.24"/>',
  ...
  leaf: '<path d="M11 20A7 7 0 0 1 9.8 6.1C15.5 5 17 4.48 19 2c1 2 2 4.18 2 8 0 5.5-4.78 10-10 10"/><path d="M2 21c0-3 1.85-5.36 5.08-6"/>',
  ...
  star: '<path d="M11.53 2.3a.53.53 0 0 1 .95 0l2.4 4.87 5.37.78c.44.06.62.6.3.91l-3.88 3.79.92 5.35c.07.44-.39.77-.78.56L12 16.03l-4.8 2.53c-.4.2-.86-.12-.79-.56l.92-5.35-3.88-3.79a.53.53 0 0 1 .3-.91l5.36-.78z"/>',
  plus: '<path d="M5 12h14"/><path d="M12 5v14"/>',
  minus: '<path d="M5 12h14"/>',
  alert: '<path d="m21.73 18-8-14a2 2 0 0 0-3.48 0l-8 14A2 2 0 0 0 4 21h16a2 2 0 0 0 1.73-3"/><path d="M12 9v4"/><path d="M12 17h.01"/>',
  bank: '<path d="M3 21h18"/><path d="M5 21V10"/><path d="M19 21V10"/><path d="M9 21v-6h6v6"/><path d="m2 10 10-7 10 7z"/>',
...
function icon(name, size) {
  const d = ICONS[name];
  if (!d) return "";
  return `<svg class="ic-svg" width="${size || 20}" height="${size || 20}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${d}</svg>`;
}
// dolu yıldız (puanlama için)
function starIcon(size) {
  return `<svg width="${size || 16}" height="${size || 16}" viewBox="0 0 24 24" fill="currentColor" stroke="none" aria-hidden="true">${ICONS.star}</svg>`;
}
function stars(n, size) { return Array.from({ length: 5 }, (_, i) => `<span style="opacity:${i < Math.round(n) ? 1 : .25}">${starIcon(size)}</span>`).join(""); }
// içerikte ikon adı yerine emoji kalmışsa onu göster (geriye uyum)
function iconOrRaw(v, size) { return ICONS[v] ? icon(v, size) : (v || ""); }
